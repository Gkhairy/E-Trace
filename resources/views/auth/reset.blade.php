...
<head>
    ...
    <title>Reset Password — E-Trace</title>
...
<body class="bg-slate-100 min-h-screen flex items-center justify-center p-4 text-slate-900">
    <div class="w-full max-w-md">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
            ...
            <p class="text-sm text-slate-500 mb-6">Kami mengirim kode 6 digit ke <b class="text-slate-700">{{ $email }}</b>. Masukkan kode & password baru kamu.</p>
            ...
            <form method="POST" action="/reset-password" class="space-y-3">
                @csrf
                <input name="code" inputmode="numeric" maxlength="6" autofocus required placeholder="Kode 6 digit"
                    class="w-full text-center tracking-[0.4em] text-xl font-bold px-4 py-3 rounded-xl bg-slate-100 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none">
                <input name="password" type="password" required placeholder="Password baru (min. 8 karakter)"
                ...
                <input name="password_confirmation" type="password" required placeholder="Ulangi password baru"
                    class="w-full px-4 py-3 rounded-xl bg-slate-100 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none text-sm">
                <button class="w-full py-3 rounded-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition shadow-sm mt-1">Ganti Password</button>
            </form>

            <form method="POST" action="/reset-password/resend" class="mt-4 text-center">
                @csrf
    ...
    <script>
        (function () {
            ...
            if (!btn) return;
            function tick() {
                if (remaining <= 0) { btn.disabled = false; timer.classList.add('hidden'); return; }
                btn.disabled = true; timer.classList.remove('hidden');
                secs.textContent = remaining; remaining -= 1; setTimeout(tick, 1000);
            }
            tick();
        })();
    </script>
